 ...
 */
async function executeCompact(ctx, agents, request, signal, onProgress, resolvedCompaction) {
  const resolved = resolveAgent(agents, request.sessionId)
  const agent = resolved.agent
  // Prefer the compaction reference captured at plugin activation (the same
  // seam `/compact` uses). Fall back to the agent's own context, then to a
  // request-time lookup, and report exactly which source is available.
  const rootCompaction = resolvedCompaction !== undefined
    ? resolvedCompaction
    : (ctx && typeof ctx.get === 'function' ? ctx.get('compaction') : undefined)
...
function apply(ctx, config) {
  ...
  // Capture the compaction seam at activation, like `/compact` does.
  const compactionAtApply = (typeof ctx.get === 'function') ? ctx.get('compaction') : undefined
  ...
  const handle = () => {
    ...
    handling = (async () => {
      ...
      try {
        ...
        const result = await executeCompact(ctx, ctx.agents, request, controller.signal, onProgress, compactionAtApply)
